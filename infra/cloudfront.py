...
def setup_cloudfront(website_bucket: aws.s3.Bucket) -> aws.cloudfront.Distribution:
  """Set up cloudfront and permissions."""
  ...
  bucket_policy = pulumi.Output.all(website_bucket.arn, oai.id).apply(
    lambda args: json.dumps({
      ...
      "Statement": [{
        "Effect": "Allow",
        "Principal": {
          "AWS": f"arn:aws:iam::cloudfront:user/CloudFront Origin Access Identity {args[1]}",
        },
        "Action": "s3:GetObject",
        "Resource": f"{args[0]}/*",
      }],
